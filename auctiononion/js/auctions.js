...
let startingPrices =[10000,20000,13000,15000,15000,170000,60000,40000,17000,15000,60000,40000,50000,6000,20000,60000]
...
let details = ["The best crop you can get at this rate\n weight is 100 kg " ];
let secondaryImages = [];
...
function generateRandomAuctions() {
  ...
  document.querySelectorAll(".card > img").forEach(img => {
    var a = Math.floor(Math.random()*g.length);
    console.log(a)
    img.src = g[a];
    primaryImages.push(img.src);
    secondaryImages.push(img.src);
    details.push("The best crop you can get at this rate\n weight is 100 kg ");
  });
...
for (let i = 0; i < startingPrices.length; i++) {
  ...
  startPrices.push({
    bid0: {
      bidder: String(i),
      amount: 10+Math.floor(Math.random()*10)*100 ,
...
function placeBid() {
  ...
  let amount = Number(amountElement.value)/100
...
function dataListener() {
  ...
  db.collection("auction-live").doc("items").onSnapshot(function (doc) {
    ...
    for (key in data) {
      ...
      let currPound = Number.parseFloat(bids["bid" + bidCount]*100).toFixed(2)
      console.log(currPound)
      // Check if the user is winning
      if (auth.currentUser) {
        let userWinning = bids["bid" + bidCount + "-user"] == auth.currentUser.uid
      }
      // Add bid data to HTML
      cb.innerHTML = "Ruppess  " + numberWithCommas(currPound) + " [" + bidCount + " bid" + (bidCount != 1 ? "s" : "") + "]"
    }
  })
}
